...
from constants import e, kB, kBeV
...
class SET:
    ...
    def rate(self, energy_diff, mu, distribution: callable):
        ...
        :return:
        """
        return self.RL * distribution((energy_diff - mu) * self.beta_ev) + self.RR * distribution((energy_diff - mu) * self.beta_ev)
        ...
        for i in range(len(self.Vg)):
            for j in range(len(self.Vd)):
                ...
                for n in range(len(energy_diff_arr)):
                    on = probability_vector[n] * self.RR * fermi((energy_diff_arr[n] - self.muR) * self.beta_ev)
                    off = probability_vector[n + 1] * self.RR * (1 - fermi((energy_diff_arr[n] - self.muR) * self.beta_ev))

                    current_matrix[i, j] += e * on
